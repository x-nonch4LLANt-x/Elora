import { s as store_get, e as ensure_array_like, a as attr, d as attr_class, c as stringify, u as unsubscribe_stores } from "./index2.js";
import { f as filteredProducts, a as activeCategory, s as sortBy } from "./products.js";
...
function CatalogPage$1($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    $$renderer2.push(`<div class="flex flex-col min-h-screen bg-background-light dark:bg-background-dark pb-24"><div class="sticky top-16 z-40 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-sm px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between"><h1 class="text-lg font-bold text-slate-900 dark:text-white">${escape_html(store_get($$store_subs ??= {}, "$t", t)("nav.catalog"))} <span class="text-slate-400 text-sm font-normal">(${escape_html(store_get($$store_subs ??= {}, "$filteredProducts", filteredProducts).length)})</span></h1> <button class="flex items-center gap-2 text-sm font-medium text-primary bg-primary/10 px-3 py-1.5 rounded-full"><span class="material-symbols-outlined text-[18px]">filter_list</span> Filter</button></div> <div class="grid grid-cols-1 gap-4 p-4"><!--[-->`);
    const each_array = ensure_array_like(store_get($$store_subs ??= {}, "$filteredProducts", filteredProducts));
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      let product = each_array[$$index];
      $$renderer2.push(`<div class="flex gap-4 p-3 bg-white dark:bg-surface-dark rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 relative"><a${attr("href", `/products/${stringify(product.id)}`)} class="absolute inset-0 z-0 rounded-2xl"${attr("aria-label", `View ${stringify(product.name)}`)}></a> <button class="absolute top-2 right-2 z-20 w-8 h-8 bg-white/90 dark:bg-black/50 backdrop-blur rounded-full flex items-center justify-center text-slate-900 dark:text-white shadow-sm active:scale-90 transition-all"><span class="material-symbols-outlined text-[18px]">visibility</span></button> <div class="w-32 h-32 shrink-0 bg-slate-100 dark:bg-slate-800 rounded-xl overflow-hidden relative z-10 pointer-events-none"><img${attr("src", product.image)}${attr("alt", product.name)} class="w-full h-full object-cover mix-blend-multiply dark:mix-blend-normal"/></div> <div class="flex flex-col justify-between py-1 flex-1 z-10 pointer-events-none"><div><div class="flex justify-between items-start"><h3 class="font-bold text-slate-900 dark:text-white line-clamp-2 leading-tight">${escape_html(product.name)}</h3> <button class="text-slate-400 hover:text-red-500 transition-colors pointer-events-auto"><span${attr_class(`material-symbols-outlined text-[20px] ${stringify(store_get($$store_subs ??= {}, "$wishlist", wishlist).includes(product.id) ? "text-red-500 fill-1" : "")}`)}>favorite</span></button></div> <p class="text-xs text-slate-500 mt-1 line-clamp-2">${escape_html(product.description)}</p></div> <div class="flex items-center justify-between mt-3"><span class="font-bold text-primary">${escape_html(store_get($$store_subs ??= {}, "$t", t)("currency"))}
                            ${escape_html(product.price.toLocaleString())}</span> <button class="bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-lg p-2 flex items-center justify-center active:scale-95 transition-transform pointer-events-auto"><span class="material-symbols-outlined text-[20px]">add_shopping_cart</span></button></div></div></div>`);
    }
    $$renderer2.push(`<!--]--></div> `);
    {
      $$renderer2.push("<!--[!-->");
